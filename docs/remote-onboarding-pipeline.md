...
- [ ] The baseline configurations template choice: (CP-IA, GBV). Configurations are staged in UNICEF's primero-x-configuration Git repo. If this onboard is a migration from Tier 3, pick the template that is closest to your current configuration. In case you want to use another repository, it should not be a private repository, because we do not handle 'secrets' in our inventory file.
